    ...
    for dataset_dir in dataset_dirs:
        ...
        for problem in tqdm(problems):
            batched_fixed_image = normalize_batched_image(
                np.expand_dims(read_problem("fixed_images", problem), axis=0).astype(
                    np.float32
                )
            )
            batched_moving_image = normalize_batched_image(
                np.expand_dims(read_problem("moving_images", problem), axis=0).astype(
                    np.float32
                )
            )
            ...
            moving_image_roi_tf = tf.cast(
                tf.expand_dims(moving_roi, axis=0), dtype=tf.float32
            )
            warping = layer.Warping(
                fixed_image_size=target_image_shape,
            ...
            warped_moving_centroids = reformat(
                compute_centroid_labels(warped_moving_roi)
            )
            ...
            centroid_distance[dataset_name][problem] = compute_score(
                CentroidDistScore, fixed_centroids, warped_moving_centroids
            )
            ...
            registered[dataset_name][problem]["warped_moving_image"] = (
            ...
            registered[dataset_name][problem]["warped_moving_centroids"] = (
                warped_moving_centroids
            )

    if write_scores:
        write_to_json(centroid_distance, "centroid_distances", "scores")
        write_to_json(ncc_scores, "ncc_scores", "scores")
