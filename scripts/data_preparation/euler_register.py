...
class EulerRegistrationProcessor:
    def __init__(
        ...
        if self.euler_search:
            self.memory_dict_xy, self._memory_dict_xy = self._initialize_memory_dict()
    ...
    def _initialize_memory_dict(self):
        ...
        y_translation_range_xy = np.sort(
            np.concatenate(
                (
        ...
        theta_rotation_range_xy = np.sort(
            np.concatenate(
                (
                    np.linspace(0, 19, 20),
        ...
        memory_dict_xy = initialize(
            np.zeros(
                (x_dim // self.downsample_factor, y_dim // self.downsample_factor)
            ).astype(np.float32),
            np.zeros(
                (x_dim // self.downsample_factor, y_dim // self.downsample_factor)
            ).astype(np.float32),
    ...
    def process_dataset_type(
        self, dataset_type: str, problem_dict: Dict[str, List[str]]
    ):
        ...
        for dataset_name, problems in problem_dict.items():
            ...
            self.process_dataset(
                dataset_name, problems, f"{dataset_type_dir}/nonaugmented"
            )
    ...
    def process_dataset(
        self, dataset_name: str, problems: List[str], dataset_type_dir: str
    ):
        ...
        with (
            h5py.File(f"{save_path}/moving_images.h5", "w") as hdf5_m_file,
            ...
            for problem in tqdm(problems):
                ...
                if len(processed_image_dict) != 0:
                    hdf5_f_file.create_dataset(
                        problem, data=processed_image_dict["fixed_image"]
                    )

                    hdf5_m_file.create_dataset(
                        problem, data=processed_image_dict["moving_image"]
                    )
                    if self.euler_search:
                        write_to_json(self.outcomes, "eulergpu_outcomes")
    ...
    def process_problem(
        ...
        if not self.euler_search:
            return self.simply_crop(problem_id, dataset_path, hdf5_m_file, hdf5_f_file)
        else:
            return self.crop_and_transform(
                problem_id, dataset_path, hdf5_m_file, hdf5_f_file
            )
    ...
    def simply_crop(
        ...
        try:
            resized_fixed_image_xyz = filter_and_crop(
                fixed_image_T, fixed_image_median, self.target_image_shape
            )
            resized_moving_image_xyz = filter_and_crop(
                moving_image_T, moving_image_median, self.target_image_shape
            )
    ...
    def crop_and_transform(
        ...
        downsampled_resized_fixed_image_xy = max_intensity_projection_and_downsample(
            resized_fixed_image_xyz, self.downsample_factor, projection_axis=2
        ).astype(np.float32)
        downsampled_resized_moving_image_xy = max_intensity_projection_and_downsample(
            resized_moving_image_xyz, self.downsample_factor, projection_axis=2
        ).astype(np.float32)

        # update the memory dictionary for grid search on x-y image
        self.memory_dict_xy["fixed_images_repeated"][:] = (
            torch.tensor(
        ...
        self.memory_dict_xy["moving_images_repeated"][:] = (
            torch.tensor(
        ...
        self.outcomes[problem_id]["registered_image_xy_gncc"] = best_score_xy.item()
        self.outcomes[problem_id]["registered_image_yz_gncc"] = calculate_gncc(
            resized_fixed_image_xyz.max(0), transformed_moving_image_xyz.max(0)
        ).item()
        self.outcomes[problem_id]["registered_image_xz_gncc"] = calculate_gncc(
            resized_fixed_image_xyz.max(1), transformed_moving_image_xyz.max(1)
        ).item()
