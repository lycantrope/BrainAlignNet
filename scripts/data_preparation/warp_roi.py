...
class ROIWarper(ImageWarper):
    ...
    def _preprocess_image_roi(
        ...
        roi_path = f"{self.label_path}/{self.dataset_name}/register_labels/{self.registration_problem}"
        fixed_image_roi = (
            nib.load(f"{roi_path}/img_fixed.nii.gz").get_fdata().astype(np.float32)
        )
        moving_image_roi = (
            nib.load(f"{roi_path}/img_moving.nii.gz").get_fdata().astype(np.float32)
        )

        if self.nonzero_labels(fixed_image_roi) and self.nonzero_labels(
            moving_image_roi
        ):
            resized_fixed_image_roi = self._resize_image_roi(
                fixed_image_roi, self.CM_dict[self.problem_id]["fixed"]
            )
            resized_moving_image_roi = self._resize_image_roi(
                moving_image_roi, self.CM_dict[self.problem_id]["moving"]
            )
            if simply_crop:
                return {
    ...
    def _resize_image_roi(
        self, image_roi: np.ndarray, image_CM: List[int]
    ) -> np.ndarray:
        return get_cropped_image(image_roi, image_CM, self.image_shape, -1).astype(
            np.float32
        )
...
def generate_rois(
    ...
    for dataset_type, datasets in dataset_types.items():
        all_bad_problems[dataset_type] = generate_roi(
            datasets, dataset_type, warper, save_directory, problem_dict, simply_crop
        )
    write_to_json(all_bad_problems, "bad_registration_problems")
...
def generate_roi(
    ...
    for dataset in datasets:
        ...
        with (
            h5py.File(f"{label_path}/moving_rois.h5", "w") as h5_m_file,
            ...
            for problem in tqdm(problems):
                ...
                if len(label_dict) > 0:
                    ...
                    h5_m_file.create_dataset(problem, data=moving_image_roi)
                    fixed_image_roi = label_dict["fixed_image_roi"]
                    h5_f_file.create_dataset(problem, data=fixed_image_roi)
