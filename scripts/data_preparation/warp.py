...
import torch
from data_utils import get_cropped_image, get_image_T, locate_dataset
...
class ImageWarper:
    def __init__(
        ...
        if simply_crop:
            self.CM_dict = self._load_json(f"resources/center_of_mass_{tag}.json")
        else:
            self.CM_dict = self._load_json(f"resources/center_of_mass_{tag}.json")
    ...
    def get_deepreg_outputs(self, target: str = "all") -> Union[dict, np.ndarray, None]:
    ...
    def _warp_moving_image_roi(
        ...
        moving_image_roi_tf = tf.cast(
            tf.expand_dims(moving_image_roi, axis=0), dtype=tf.float32
        )
        # TODO: set `batch_size` required by the latest centroid_label network
        warping = layer.Warping(
            fixed_image_size=self.image_shape, interpolation="nearest"
        )
        if intput_ddf == None:
    ...
    def _load_images(self, network_output_path: str) -> Dict[str, np.ndarray]:
        ...
        warped_moving_image = nib.load(
            f"{network_output_path}/pred_fixed_image.nii.gz"
        ).get_fdata()
        fixed_image = nib.load(f"{network_output_path}/fixed_image.nii.gz").get_fdata()
        moving_image = nib.load(
            f"{network_output_path}/moving_image.nii.gz"
        ).get_fdata()
    ...
    def _preprocess_image_roi(self) -> Dict[str, np.ndarray]:
        ...
        resized_fixed_image_roi = self._resize_image_roi(
            fixed_image_roi_path, self.CM_dict[self.problem_id]["fixed"]
        )
        resized_moving_image_roi = self._resize_image_roi(
            moving_image_roi_path, self.CM_dict[self.problem_id]["moving"]
        )
    ...
    def _resize_image_roi(self, image_roi_path: str, image_CM: List[int]) -> np.ndarray:
        ...
        return get_cropped_image(image_roi_T, image_CM, self.image_shape, -1).astype(
            np.float32
        )

    def _euler_transform_image_roi(
        self, moving_image_roi: np.ndarray, interpolation: str = "nearest"
    ) -> np.ndarray:
        ...
        return self._apply_euler_parameters(
            moving_image_roi, _memory_dict_xy, interpolation
        )

    def _initialize_memory_dict(
        self, dim_1: int, dim_2: int, dim_3: int
    ...
    def _apply_euler_parameters(
        ...
        moving_image_roi = self._adjust_image_shape(moving_image_roi, dimension)
        if dimension == "xy":
            axis = 2
        elif dimension == "xz":
            axis = 1
        ...
        translated_moving_image_roi = self._translate_image(
            self._adjust_image_shape(transformed_moving_image_roi, "xy")
        )
    ...
    def _adjust_image_shape(
        ...
        x_dim, y_dim, z_dim = self.image_shape
        reshaping_dict = {
            (y_dim, z_dim, x_dim): {"xy": (2, 0, 1), "xz": (2, 1, 0), "yz": (0, 1, 2)},
            (x_dim, z_dim, y_dim): {"xy": (0, 2, 1), "yz": (2, 1, 0), "xz": (0, 1, 2)},
            (x_dim, y_dim, z_dim): {"yz": (1, 2, 0), "xz": (0, 2, 1), "xy": (0, 1, 2)},
        }
