...
class Sampler:
    ...
    def __init__(
        ...
        self.output_dict = {"train": dict(), "valid": dict(), "test": dict()}
        ...
        for dataset_type, dataset_names in self.problem_dict.items():
            for dataset_name in tqdm(dataset_names):
                sampled_problems = random.sample(
                    self.problem_dict[dataset_type][dataset_name], num_problems
                )
    ...
    def create_problems_to_register(self, output_file_name: str, num_problems: int):
        ...
        assert (
            self.neuron_roi_path is not None
        ...
        for dataset_type, dataset_names in self.dataset_dict.items():
            for dataset_name in tqdm(dataset_names):
                ...
                moving_time_points = random.choices(
                    list(
                        filter(lambda item: item not in fixed_time_points, time_points)
                    ),
                    k=num_problems,
                )
                self.output_dict[dataset_type][dataset_name] = [
                    f"{moving_t}to{fixed_t}"
                    for moving_t, fixed_t in list(
                        zip(moving_time_points, fixed_time_points)
                    )
                ]
        write_to_json(self.output_dict, output_file_name)
    ...
    def _get_all_problems(self, dataset_name: str) -> List[str]:
        ...
        if os.path.exists(f"{dataset_path}/registration_problems.txt"):
            lines = open(f"{dataset_path}/registration_problems.txt", "r").readlines()
    ...
    def _sample_registration_problems(
        self, problems: List[str], cutoff: int = 600
    ) -> List[str]:
        ...
        for problem in problems:
            interval = abs(int(problem.split("to")[0]) - int(problem.split("to")[1]))
        ...
        for interval, problems in interval_to_problems_dict.items():
            if interval > cutoff:
                sampled_problems += random.sample(problems, int(0.8 * len(problems)))
            else:
                sampled_problems += random.sample(problems, int(0.5 * len(problems)))
