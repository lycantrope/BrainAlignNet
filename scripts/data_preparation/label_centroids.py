...
class CentroidLabel:
    ...
    def create_one_dataset_labels(
        self, dataset_name: str, dataset_type: str, max_centroids: int = 200
    ):
        ...
        save_directory = (
            f"{self.dataset_path}/{dataset_type}/nonaugmented/{dataset_name}"
        )
        ...
        with (
            h5py.File(f"{save_directory}/moving_labels.h5", "w") as hdf5_m_file,
            ...
            for problem in tqdm(problems):
                ...
                fixed_centroids = self._compute_centroids_3d(fixed_roi, max_centroids)
                moving_centroids = self._compute_centroids_3d(moving_roi, max_centroids)
    ...
    def _compute_centroids_3d(
        self, image: NDArray[np.int32], max_centroids: int
    ) -> NDArray[np.int32]:
        ...
        centroids = (
            np.zeros((max_centroids, 3), dtype=np.int32) - 1
