...
class FPImageWarper(EulerRegistrationProcessor):
    def __init__(self, *args, **kwargs):
        ...
        self.CM_dict = self._load_json("resources/center_of_mass.json")
        self.euler_parameters_dict = self._load_json("resources/euler_parameters.json")
    ...
    def process_problem(
        ...
        euler_transformed_moving_image = self._euler_transform_image(
            resized_moving_image_xyz, problem_id
        )
        return {
    ...
    def _euler_transform_image(
        self, moving_image: np.ndarray, problem_id: str, interpolation: str = "bilinear"
    ):
        ...
        return self._apply_euler_parameters(
            moving_image, _memory_dict_xy, interpolation, problem_id
        )
    ...
    def _apply_euler_parameters(
        ...
        translated_moving_image_roi = self._translate_image(
            self._adjust_image_shape(transformed_moving_image_roi, "xy"), problem_id
        )
    ...
    def _translate_image(self, image: np.ndarray, problem_id: str) -> np.ndarray:
