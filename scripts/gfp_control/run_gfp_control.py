...
from data_utils import write_to_json
...
def register_all_swf360_problems(
    ...
    def get_swf360_problems(dataset):
        ...
        return json.load(open(f"resources/registration_problems_{dataset}.json"))[
            "train"
        ][dataset_name]
    ...
    write_to_json(ncc_scores_dict, f"{dataset}_ncc_{network_version}", "scores")
...
def register_single_image_pair(
    ...
    def compute_score(score_class, *args):
        if len(args) == 2:
            ...
            return score_class(img_size=target_image_shape).call(args[0]).numpy()[0]
    ...
    def _warp_ch2_with_ddf(problem, fixed_image, moving_image):
        ...
        if batched_moving_image.dtype == np.float32:
            ...
            ncc = calculate_ncc(
                pred_fixed_image.squeeze(), batched_fixed_image.numpy().squeeze()
            )
    ...
    def _warp_ch1_with_ddf(problem, ch2_ddf, fixed_image, moving_image):
        ...
        warping = layer.Warping(
            fixed_image_size=batched_fixed_image.shape[1:4], batch_size=1
        )
        ...
        ncc = calculate_ncc(
            warped_moving_image.numpy().squeeze(), batched_fixed_image.numpy().squeeze()
        )
    ...
    all_outputs["ch1"] = _warp_ch1_with_ddf(
        problem, all_outputs["ch2"]["ddf"], ch1_fixed_image, ch1_moving_image
    )
...
def read_problem_h5(dataset, problem, channel):
    ...
    dataset_name = dataset_names[dataset]
    dataset_path = f"/data3/prj_register/{dataset}_swf360_ch{channel}/train/nonaugmented/{dataset_name}"
...
def normalize_batched_image(batched_image, eps=1e-7):
    ...
    batched_image = batched_image / tf.maximum(
        tf.cast(max_vals - min_vals, tf.float32), eps
    )
