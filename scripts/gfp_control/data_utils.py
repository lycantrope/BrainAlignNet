...
jl.include(os.fspath(Path(__file__).with_name("adjust_image.jl")))


def adjust_image_size(image, center, target_dim):
    return np.asarray(jl.adjust_image_size(image, center, target_dim))
...
def locate_dataset(dataset_name: str):
    ...
    if dataset_name == "2022-01-06-01":
        return f"{swf360_path}/{dataset_name}-SWF360-animal1-610LP_newunet_output"
        ...
        return f"{swf360_path}/{dataset_name}-SWF360-animal2-610LP_diffnorm_ckpt287"
...
def filter_and_crop(
    image_T: np.ndarray, image_median: float, target_image_shape: Tuple[int, int, int]
) -> np.ndarray:
    ...
    return get_cropped_image(
        filtered_image_T, filtered_image_CM, target_image_shape, -1
    ).astype(np.float32)


def filter_image(image: np.ndarray, threshold: float) -> np.ndarray:
...
def extract_all_problems(dataset_name: str, problem_file_path: str) -> None:
